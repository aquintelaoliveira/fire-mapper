...
{
    [Collection("WeakStoreFixture collection")]
    public class WeakDataSourceTest
    {
        readonly ITestOutputHelper output;
        private readonly WeakStoreFixture fix;
        private readonly WeakDataSource studentsDb;

        public WeakDataSourceTest(ITestOutputHelper output, WeakStoreFixture fix)
        {
            this.output = output;
            this.fix = fix;
        ...
        public void AddGetAndDeleteAndGetAgain()
        {
            ...
            var exception = Assert.Throws<KeyNotFoundException>(
                () => studentsDb.GetById(st.Number));
            Assert.Equal($"There is no entry in database for key {st.Number}!", exception.Message);
        }
